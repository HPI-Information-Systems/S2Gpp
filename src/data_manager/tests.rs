...
fn test_data_management() {
    ...
    let expected_main_phase_space = arr3(&[[[2.18241731, 4.45332896, 2.18779886],
    ...
    let expected_main_data_ref = arr3(&[[[0.03510282, 0.23744748, 0.0171921 ],
       ...
       [[0.15549958, 0.35270244, 0.13633183],
        ...
        [0.15549958, 0.35270244, 0.13633183],
        [0.15549958, 0.35270244, 0.13633183]]]);
    let expected_sub_phase_space = arr3(&[[[3.8206407 , 2.36796312, 2.04560368],
    ...
    arr.into_par_iter().for_each(|p| start_reading(p.ip, p.seeds.clone(), p.main, p.expected_phase_space.clone(), p.expected_data_ref.clone()));
}
...
async fn start_reading(ip_address: SocketAddr, seed_nodes: Vec<SocketAddr>, main: bool, expected_phase_space: ArcArray<f32, Ix3>, expected_data_ref: ArcArray<f32, Ix3>) {
    ...
    println!("phase_space {:?}", data_result.phase_space.as_ref().unwrap().shape());
    println!("data_ref {:?}", data_result.data_ref.as_ref().unwrap().shape());

    close_l1(&data_result.phase_space.as_ref().unwrap().slice(s![..3, .., ..]).to_owned(), &expected_phase_space, 0.0005);
    close_l1(&data_result.data_ref.as_ref().unwrap().slice(s![..3, .., ..]).to_owned(), &expected_data_ref, 0.0005);
}
